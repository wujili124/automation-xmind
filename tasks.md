...
- [x] XMind标识符识别优化
  ...
  - 实现动态标识符识别，提取用户上传XMind文件中包含的全部标识符(这就需要映射关系都要枚举出来)
  - 改进标识符映射逻辑，提高识别准确率和兼容性
  - 添加未知标识符的展示和选择功能，提升用户体验
  - 兼容下游导出Excel选中标识符的功能及参数，确保导出数据准确无误
  - 优化方案：
    - 增强标识符提取功能：
      - 实现对XMind 8和XMind Zen格式的全面支持
      - 添加对markers、icons、labels等多种标识符字段的解析
      - 支持提取XML和JSON格式中的所有可能标识符
    - 完善标识符映射系统：
      - 创建完整的标识符映射数据库，包含常见XMind标识符的所有变体
      - 实现智能模糊匹配算法，提高识别准确率
    - 改进用户界面：
      - 在上传XMind文件后，展示所有发现的标识符，包括图标预览
    - 实现标识符元数据管理：
    - 具体实现步骤：
      1. 重构`XMindAnalyzer`类，分离标识符提取和映射逻辑
      2. 创建专门的`MarkerRegistry`类管理标识符映射关系
      3. 实现更全面的标识符解析器，支持多种XMind格式
  - 实现成果：
    1. 创建了`MarkerRegistry`类，集中管理标识符映射关系
    2. 开发了`EnhancedXMindAnalyzer`类，提供更强大的标识符识别功能
    3. 实现了`XMindAnalyzerAdapter`适配器，确保与现有系统的兼容性
    4. 扩展了标识符数据库，支持更多种类的标识符
    5. 添加了未知标识符的自动识别和分类功能
    6. 实现了标识符映射的自学习能力
    7. 优化了未知标识符的显示名称，使其更加语义化（例如将"unknown-flag-red"显示为"红旗"）
    8. 在过滤XMind文件时，自动去除"unknown-"前缀，确保与XMind原始标识符兼容
